...
**继续使用 Claude Code。离开电脑后，也能从飞书回到同一个会话。**

Wirelark 是 Claude Code 安静的远程伴侣。它不会代你运行 Claude Code，也不会
取代 Claude Code TUI。它只是把飞书连接到由你在自己电脑上启动、始终归你掌控
的会话。
...
Wirelark 默默运行在后台：Claude Code 依然是你的工作区，飞书只是在你离开电脑
时，成为一个临时远程窗口。会话需要你时，从飞书响应；回到电脑后，在原来的终端
里继续同一个会话。
...
> **需要关注时通知，完成后总结，并继续现有会话。**

Wirelark 不会把每次文件读取、搜索、Shell 命令或工具调用都发送到飞书。它会
提示权限请求和 Claude 提出的问题，总结已完成的工作；对于长时间运行的任务，
还可以选择只更新同一张进度卡片。
...
向飞书机器人发送 `sessions`，然后从电脑上正在运行的 Claude Code 会话中选择
一个：
...
点击会话，或回复它的编号。之后的消息只会进入这个指定会话，不会发送到其他
会话。如果会话已经结束，Wirelark 会清除选择，不会悄悄把消息转发到别处。
...
Wirelark 提供克制的关注与完成通知、本地会话概览、对指定会话的后续指令，以及
可选的远程权限决策。卡片按钮只是快捷方式，并非必需；输入会话编号和明确的
权限回复同样有效。

## 与 Agent 网关有何不同

许多实用的 Agent 网关会把桥接程序或平台作为工作的起点：
...
如果聊天工具是主要工作区，网关是很自然的入口。Wirelark 面向的是另一种需求：
让 Claude Code 本身始终作为工作区，只在离开电脑时，通过一个安静的桥梁临时
连接进去。
...
下载匹配平台的独立二进制文件，校验其 checksum，并安装到 `~/.local/bin`。无需
维护运行时环境、容器或服务管理器。
...
扫码账号会成为该 Wirelark 安装的所有者。在由管理员统一管理的环境中，也可以
改用已有的 App ID 和 App Secret。
...
Wirelark 在本地运行一个轻量守护进程，用来维持飞书连接并识别本机上的 Claude
Code 会话。Claude Code Hooks 提供会话生命周期、需要关注和完成等事件；Claude
Channels 则提供受支持的连接方式，把飞书消息送入一个已经运行的会话。
...
Hook 行为和运维控制，请参阅[安全与运维（英文）](docs/security-and-operations.md)。

## 安全与透明

只有本地 Wirelark 桥接程序会与飞书通信。它只发送实现已配置体验所需的卡片和
消息，例如会话标识、完成内容摘录、验证结果和权限详情；不会发送终端数据流或
完整的会话记录。

Wirelark 只接受来自已配置所有者的消息。远程消息只会发送到明确选定的会话。
远程权限批准是独立设置，因为它会向该飞书身份授予真实的操作权限。
...
- **远程继续功能目前需要预览参数。** Claude Code Channels 仍处于研究预览阶段，
...
- Channels 需要通过 claude.ai 或 Console API Key 进行 Anthropic 身份验证，
  ...
  Channels。
- Claude Code 的多选 `AskUserQuestion` 提示目前无法通过 Channel 回答。Wirelark
  会通知你，但仍需回到原终端作答。
- 如需远程继续，电脑、Claude Code 会话、Wirelark 守护进程和网络连接都必须
  保持运行。
- WSL 与原生 Windows 属于两套独立安装。在 Windows 上，第一次消息确认会话的
  Channel 之前，远程状态会显示为尚未验证。

## 项目状态

克制的通知、完成总结、本地会话发现、指定会话的远程继续、可选权限决策、发布
版二进制安装和二维码引导均已实现。

下一步产品方向，是为选定会话提供可选的精简实时视图——不是终端串流，也不是
第二套 Claude Code 界面。完整设计思路请参阅
