...
- **每个会话一张卡片。** 一轮工作真正开始时卡片会自动出现，并持续原地更新，
  直到该轮结束并定格为结果。无需开启，也无需关闭。
- **精确控制会话。** 在卡片的输入框中回复，消息只会进入该会话。直接在聊天里输入时，
  消息只会进入唯一一个可继续的会话，否则原地不动 —— 绝不会进入你没有指定的会话。
  发送 `sessions` 可把每个会话的卡片重新送到对话底部。
- **对做不到的事直说。** 未启用 Companion channel 启动的会话不会显示输入框，
  卡片会用一句话说明原因。
- **中断。** 点击 **Interrupt** 或发送 `interrupt` 只会停止当前轮次，不会结束会话。
